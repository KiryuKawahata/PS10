...
```{r}
#train score
PS10_Lm <- lm(model_formula, data = train)

train_score <- RMSLE(exp(PS10_Lm$fitted.values), train$price_doc)

train_score

#Kaggle:
lm_predictions <- predict(PS10_Lm, newdata = test)

price_doc <- exp(lm_predictions)
...
train_price_doc_LASSO <- predict(LASSO_fit, newx = predictor_matrix_train, s = lambda_star_1SE) 

train_score <- RMSLE(exp(train_price_doc_LASSO), train$price_doc)
train_score

price_doc <- predict(LASSO_fit, newx = predictor_matrix_test, s = lambda_star_1SE) 

price_doc <- exp(price_doc)

LASSO_Submission <- test %>% 
  mutate(price_doc) %>% 
  select(id, price_doc)
  
write.csv(LASSO_Submission, "LASSO_Submission.csv", row.names = F)
...
--------------------- | -------------   | -------------
lm                    |    0.2534704    |  2.63413
LASSO crossvalidated  |    0.5172888    |  0.42805
